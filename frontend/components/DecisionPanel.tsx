...
function ItemList({ label, items }: { label: string; items: string[] }) {
  ...
  return (
    <div>
      <h5 className="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">{label}</h5>
      <ul className="mt-1 list-inside list-disc space-y-0.5 text-sm text-gray-600 dark:text-gray-300">
  ...
  return (
    <article className={`rounded-lg border p-4 ${recommended ? "border-blue-400 bg-blue-50/50" : "border-gray-100"}`}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h4 className="font-semibold text-gray-900 dark:text-white">{option.action}</h4>
        {recommended && <span className="rounded-full bg-blue-600 px-2.5 py-1 text-xs font-semibold text-white">Recommended</span>}
      </div>
      <p className="mt-2 text-sm text-gray-700 dark:text-gray-300">{option.rationale}</p>
      <div className="mt-3 flex flex-wrap gap-x-6 gap-y-1 text-xs text-gray-500 dark:text-gray-400">
        <span>Confidence: <strong className="text-gray-700 dark:text-gray-300"><Percentage value={option.confidence} /></strong></span>
        <span>Risk score: <strong className="text-gray-700 dark:text-gray-300"><Percentage value={option.risk_score} /></strong></span>
      </div>
    ...
    <section className="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-surface-dark" aria-labelledby="decision-panel-title">
      <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4 dark:border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <h3 id="decision-panel-title" className="text-base font-semibold text-gray-900 dark:text-white">Decision Optimization</h3>
        </div>
      ...
      <div className="space-y-4 px-6 py-5">
        {data?.decision_rationale && (
          <div>
            <h4 className="mb-1 text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">Decision rationale</h4>
            <p className="text-sm text-gray-700 dark:text-gray-300">{data.decision_rationale}</p>
          </div>
        )}
        {data?.recommended_decision && (
          <div className="rounded-lg bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">
            Recommended action: {data.recommended_decision}
          </div>
        )}
        {options.length > 0 ? (
          <div className="space-y-3">
            <h4 className="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500">Decision options</h4>
            {options.map((option) => (
