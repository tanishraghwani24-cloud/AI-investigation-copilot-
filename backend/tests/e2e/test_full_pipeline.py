...
@pytest.mark.asyncio
async def test_full_pipeline_happy_path(mock_db):
    """Verify that a full investigation pipeline can execute asynchronously in the background."""
    transport = httpx.ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        # 1. Create Investigation
        resp = await client.post("/api/investigations?scenario=default")
        ...
        # ASGITransport waits for BackgroundTasks to finish before returning the response.
        # To poll during execution in a test, we run the POST request in a background task.
